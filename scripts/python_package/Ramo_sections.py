...
df = pd.read_table(filepath_or_buffer=filename_ramo, index_col=1, sep=r'\s+', \
...
# The file is only written when write_ramo_sec = .true. in the input namelist.
# Check the size so we fail with a clear message instead of an obscure memmap error.
expected_size = MAX_SECTIONS * MAX_EMITTERS * steps * 8 # float64
actual_size = path.getsize(filename_ramo_sec)
if actual_size != expected_size:
    raise SystemExit('ramo_current.bin is {:d} bytes but expected {:d} '
                     '(MAX_SECTIONS={:d} x MAX_EMITTERS={:d} x steps={:d} x 8). '
                     'Was the simulation run with write_ramo_sec = .true. in the input file?'
                     .format(actual_size, expected_size, MAX_SECTIONS, MAX_EMITTERS, steps))
