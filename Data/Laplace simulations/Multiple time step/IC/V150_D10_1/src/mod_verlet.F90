...
contains
  ...
  subroutine Velocity_Verlet(step)
    ...
    call Update_Particle_Acceleration()
  ...
  subroutine Update_Particle_Position(step)
    ...
    ! Electrons are moved at every time step
    call Update_Species_Position(step,species)
    ! Ions are moved at every time_step_atom time step
    if (mod(step,time_step_atom) == 0) then
      call Update_Species_Position(step,species)
    end if
    ! Atoms are currently immobile
  ...
  subroutine Update_Species_Position(step, species)
    ...
    select case (species)
      ...
      case (species_ion)
        nrFor = nrIon
        nrStep = time_step_atom*1.0d0
    end select
    nrStep2 = nrStep**2

    !$OMP PARALLEL DO PRIVATE(i, k) &
    !$OMP& SHARED(species, nrFor, particles_prev_pos, particles_cur_pos, particles_cur_vel, particles_cur_accel) &
  ...
  subroutine Update_Particle_Velocity(step)
    ...
    ! Ion velocity is updated at every time_step_atom time step
    if (mod(step,time_step_atom) == 0) then
  ...
  subroutine Update_Species_Velocity(step,species)
    ...
    integer, intent(in)              :: step, species
    integer                          :: i, j, s, nrFor, emit, sec
    double precision                 :: q, nrStep
    ...
    !$OMP& PRIVATE(i, j, s, q, emit, sec) &
    !$OMP& SHARED(species, nrFor, particles_cur_vel, particles_prev_accel, particles_cur_accel, time_step) &
    ...
    do k = 1, nrFor
      ...
      !$OMP ATOMIC UPDATE
      ramo_current(s) = ramo_current(s) + q * E_zunit * particles_cur_vel(3, i)
  ...
  subroutine Update_Particle_Acceleration()
