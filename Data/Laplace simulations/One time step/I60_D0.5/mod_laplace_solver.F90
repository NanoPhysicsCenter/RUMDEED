...
module mod_laplace_solver
    ...
    integer(kind=8) :: msglvl=0, error, analysis=11, factorization=22, solving=13
...
! -----------------------------------------------------------------------------
    ...
    subroutine Init_Laplace_Solver() ! DONE
        ...
        allocate(values(n19), col_index(n19), b(nrGridActive), iCharge(nrGridActive), oCharge(nrGridActive))
...
! -----------------------------------------------------------------------------
    ...
    subroutine Clean_Up_Laplace() ! DONE
        ...
        if (allocated(iCharge)) then deallocate(iCharge) end if
        if (allocated(laplace_field)) then deallocate(laplace_field) end if
        if (allocated(nnz_values)) then deallocate(nnz_values) end if
        if (allocated(nnz_col_index)) then deallocate(nnz_col_index) end if
        if (allocated(nnz_ia)) then deallocate(nnz_ia) end if
        if (allocated(voltage)) then deallocate(voltage) end if
        if (allocated(x)) then deallocate(x) end if
