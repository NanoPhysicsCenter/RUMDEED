...
!
  ...
  ! The integration function for Cuba
  integer function integrand_cuba(ndim, xx, ncomp, ff, userdata)
    ...
    if (field(3) < 0.0d0) then
      ...
      ff(1) = Elec_Supply_V2(par_pos, field(3))
    else
  ...
  end function integrand_cuba

  subroutine Do_Cuba_Suave(emit, N_sup)
    ! Input / output variables
    integer, intent(in)  :: emit
    ...
    userdata = emit ! Pass the number of the emitter being integraded over to the integrand as userdata

    call suave(ndim, ncomp, integrand_cuba, userdata, nvec, &
  ...
  subroutine Do_2D_MC_plain(emit, N_sup)
    integer, intent(in)  :: emit ! The emitter to do the integration on
    integer, intent(out) :: N_sup ! Number of electrons
