...
!        if (emitters(i)%delay <= step) then
!          select case (emitters(i)%type)
!            case (EMIT_CIRCLE)
!              !print *, 'Doing Circle'
...
!      end do
  ...
  subroutine Do_Photo_Emission_Circle(step, emit)
    ...
    nrTry = 0
    nrElecEmit = 0
    nrEmitted_emitters(nrEmit) = 0

    do while (nrTry <= MAX_EMISSION_TRY)
    !do i = 1, MAX_EMISSION_TRY
      if (nrElec == MAX_ELECTRONS-1) then
      ...
      r_e = emitters(emit)%dim(1) * par_pos(1)
      theta_e = 2.0d0*pi * par_pos(2)
      par_pos(1) = r_e * cos(theta_e) + emitters(emit)%pos(1)
      par_pos(2) = r_e * sin(theta_e) + emitters(emit)%pos(2)

      nrTry = nrTry + 1
      par_pos(3) = 0.0d0 * length ! Check in plane
      field = Calc_Field_at(par_pos)

      if (field =< 0.0d0) then
        par_pos(3) = 1.0d0 * length ! Place above plane
        call Add_Particle(par_pos, par_vel, species_elec, step)
        call Add_Plane_Graph_emitt(par_pos, par_vel)

        nrElecEmit = nrElecEmit + 1
        nrEmitted_emitters(nrEmit) = nrEmitted_emitters(nrEmit) + 1
        nrTry = 0
      end if
    end do
  ...
  subroutine Do_Photo_Emission_Rectangle(step, emit)
    ...
    do while (nrTry <= MAX_EMISSION_TRY)
      ...
      par_pos(1) = emitters(emit)%pos(1) + emitters(emit)%dim(1)*par_pos(1)
      par_pos(2) = emitters(emit)%pos(2) + emitters(emit)%dim(2)*par_pos(2)
      ...
      if (field(3) =< 0.0d0) then
        par_pos(3) = 1.0d0 * length !Above plane
        field = Calc_Field_at(par_pos)
